...
// Compare local file hash vs GitHub blob sha
function isFileDifferent(localPath, expectedSha) {
  if (!fs.existsSync(localPath)) return true
  const content = fs.readFileSync(localPath)
  const localHash = hashBuffer(Buffer.from(`blob ${content.length}\0`)).toString("hex")
  // GitHub tree blob sha is already SHA1 of "blob <size>\0<content>"
  return localHash !== expectedSha
}
...
export async function updateRepo() {
  ...
  for (const item of tree) {
    if (item.type === "blob") {
      ...
      const needsUpdate = isFileDifferent(localPath, item.sha)
