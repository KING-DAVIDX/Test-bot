...
export async function updateRepo() {
  ...
  let changedFiles = []
  if (lastSha) {
    changedFiles = await getChangedFiles(lastSha, latestSha)
  } else {
    const res = await api.get(`/git/trees/${latestSha}?recursive=1`)
    changedFiles = res.data.tree.filter(i => i.type === "blob").map(i => i.path)
  }
  let updated = []
