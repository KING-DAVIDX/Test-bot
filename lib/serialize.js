...
import baileys, { downloadMediaMessage, jidNormalizedUser } from "baileys"; 
import { resolveLidToJid, getGroupMetadata } from "./lid.js"; 
import { getConfig } from "./filewatcher.js"; 
import { plugins, listeners } from "./plugin.js";
...
export async function serialize(sock, msg) { 
  ...
  if (rawParticipant) {
    m.participant = resolveLidToJid(rawParticipant, groupMeta);
  } else {
    m.participant = m.isGroup ? jidOwner : m.from;
  }

  m.fromMe = [jidOwner, lidOwner].includes(m.participant);
  ...
  if (contextInfo.quotedMessage) {
    ...
    quoted.download = async () => {
        ...
        if (!allowed.includes(quoted.type)) return null;
